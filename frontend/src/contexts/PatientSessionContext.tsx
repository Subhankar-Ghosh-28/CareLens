...
import { DEMO_PATIENT_ANANYA, DEMO_DOCUMENTS, DEMO_TIMELINE, DEMO_ALERTS, DEMO_CONSENTS, DEMO_CLINICAL_SUMMARY } from '../data/demoData';
import {
  consentService,
  ocrService,
...
interface PatientSessionContextType {
  ...
  saveAnswer: (questionId: string, questionText: string, category: string, answerText: string, inputType?: 'voice' | 'touch' | 'text') => Promise<void>;
  uploadDocument: (file: File, category: string) => Promise<MedicalDocument>;
  removeDocument: (id: string) => void;
  grantConsent: (type: 'HISTORY_CAPTURE' | 'DOCUMENT_DIGITIZATION' | 'STAFF_SHARING', lang?: string) => Promise<void>;
...
export const PatientSessionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [interviewAnswers, setInterviewAnswers] = useState<InterviewAnswerItem[]>([
    {
  ...
  const addInterviewAnswer = (item: InterviewAnswerItem) => {
    setInterviewAnswers(prev => {
      const filtered = (prev || []).filter(a => a.questionId !== item.questionId);
  ...
  const initialDocuments: MedicalDocument[] = DEMO_DOCUMENTS.map(doc => ({
    ...doc,
    title: (doc as any).title || (doc.category === 'Prescription' ? 'Cardiology Prescription (Dr. Nair)' : doc.category === 'Lab Report' ? 'Metropolis Glycemic Profile' : 'Hospital Discharge Summary'),
    fileName: (doc as any).fileName || doc.filename || 'document.pdf',
    documentType: (doc as any).documentType || doc.category || 'Prescription',
    confidenceScore: (doc as any).confidenceScore || (doc.confidence ? doc.confidence / 100 : 0.95),
    extractedData: (doc as any).extractedData || {
      medications: doc.category === 'Prescription' ? [{ name: 'Metformin 500mg' }, { name: 'Telmisartan 40mg' }, { name: 'Atorvastatin 10mg' }] : [],
      labResults: doc.category === 'Lab Report' ? [{ testName: 'HbA1c', value: '7.8', unit: '%' }, { testName: 'Fasting Glucose', value: '146', unit: 'mg/dL' }] : []
    }
  }));

  const [uploadedDocuments, setUploadedDocuments] = useState<MedicalDocument[]>(initialDocuments);
  ...
  const [clinicalTrack, setClinicalTrackState] = useState<'MODERN_MEDICINE' | 'AYUSH'>('MODERN_MEDICINE');
  ...
  const [clinicalSummary, setClinicalSummary] = useState<any>(DEMO_CLINICAL_SUMMARY);
  ...
  const addRedFlag = (alert: any) => {
    const formattedAlert: ClinicalAlert = {
      ...
      priority: alert.priority || (alert.severity === 'HIGH' ? 'CRITICAL' : 'ATTENTION'),
    ...
    setAlerts(prev => {
      const existing = prev || [];
      if (existing.some(a => a.trigger === formattedAlert.trigger)) return existing;
  ...
  const [secondsUntilTimeout, setSecondsUntilTimeout] = useState<number>(INACTIVITY_TIMEOUT_SECONDS);
      ...
      setSecondsUntilTimeout(prev => {
        if (prev <= 1) {
          ...
          auditService.logEvent({
            ...
            actorRole: 'System AI',
            action: 'Kiosk session terminated automatically due to inactivity timeout',
  ...
  const saveAnswer = async (
    ...
    setRecordedAnswers(prev => [...prev.filter(a => a.questionId !== questionId), record]);

    // Check red flags
    const triggeredAlerts = await clinicalHistoryService.evaluateRedFlags(nextAnswers);
  ...
  const uploadDocument = async (file: File, category: string): Promise<MedicalDocument> => {
    ...
    auditService.logEvent({
      ...
      patientId: patient.id,
      metadata: { docId: result.document.id, confidence: result.document.confidence }
    });
  ...
  const grantConsent = async (type: 'HISTORY_CAPTURE' | 'DOCUMENT_DIGITIZATION' | 'STAFF_SHARING', lang: string = 'en') => {
  ...
  const hasRequiredConsents = consents.filter(c => c.status === 'GRANTED').length >= 3;

  const refreshTimeline = async () => {
    const events = await timelineService.generateTimeline(patient.id, uploadedDocuments, answers);
    setTimeline(events);
  };
...
export const usePatientSession = () => {
  ...
  if (!context) throw new Error('usePatientSession must be used within PatientSessionProvider');
  return context;
};
