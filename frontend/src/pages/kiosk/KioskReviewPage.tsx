  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto w-full space-y-6">
        ...
        {/* Priority Attention Alerts if any */}
        {(redFlags || []).length > 0 && (
          <div className="p-5 rounded-3xl bg-rose-50 border-2 border-rose-300 space-y-3">
            <div className="flex items-center gap-2 text-rose-900 font-bold text-sm">
              <AlertTriangle className="w-5 h-5 text-rose-600" />
              <span>Priority Attention Items (Sent to OPD Triage Staff)</span>
            </div>
            <div className="space-y-2">
              {(redFlags || []).map((rf: any) => (
                <div key={rf.id || rf.alertId} className="p-3 bg-white rounded-xl border border-rose-200 text-xs">
                  <div className="flex items-center justify-between font-bold text-rose-950 mb-1">
                    <span>{rf.title || rf.trigger}</span>
                    <RedFlagBadge severity={rf.severity} priority={rf.priority} />
                  </div>
                  <p className="text-slate-600">{rf.description || rf.wording}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-5 rounded-3xl bg-white border border-slate-200 shadow-2xs space-y-3">
            ...
            <p className="text-xs text-slate-700 leading-relaxed font-medium">
              {typeof clinicalSummary?.chiefComplaint === 'string'
                ? clinicalSummary.chiefComplaint
                : clinicalSummary?.chiefComplaint?.content || 'Chest discomfort with mild sweating for 2 days. Occasional radiating pain to left shoulder.'}
            </p>
            <div className="text-[11px] text-slate-500 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
              HPI: {(typeof clinicalSummary?.historyOfPresentIllness === 'string'
                ? clinicalSummary.historyOfPresentIllness
                : clinicalSummary?.historyOfPresentIllness?.content || 'Patient reports episodic retrosternal pressure...').substring(0, 140)}...
            </div>
          </div>
            ...
            <div className="space-y-2">
              <div className="text-xs">
                ...
                <span className="text-slate-600">
                  {Array.isArray(clinicalSummary?.currentMedications)
                    ? clinicalSummary.currentMedications.map((m: any) => `${m.name} ${m.dosage || ''}`).join(', ')
                    : typeof clinicalSummary?.medications === 'object'
                    ? clinicalSummary.medications?.content || 'Metformin 500mg, Telmisartan 40mg'
                    : 'Metformin 500mg, Telmisartan 40mg'}
                </span>
              </div>
              <div className="text-xs">
                <span className="font-bold text-slate-800 block">Known Allergies:</span>
                <span className="text-rose-700 font-semibold">
                  {Array.isArray(clinicalSummary?.allergies)
                    ? clinicalSummary.allergies.join(', ')
                    : typeof clinicalSummary?.allergies === 'object'
                    ? (clinicalSummary as any).allergies?.content || 'Penicillin (Severe cutaneous rash)'
        ...
        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between text-xs text-slate-600">
          <div className="flex items-center gap-2">
            ...
            <span>
              <strong>{(uploadedDocuments || []).length} Medical Documents</strong> attached for doctor examination.
            </span>
          </div>
          <span className="font-semibold text-teal-800">OCR Verified</span>
        </div>

        {/* Clarification Box */}
        {clarificationOpen ? (
          <form onSubmit={handleSaveClarification} className="p-5 rounded-3xl bg-white border border-teal-300 shadow-sm space-y-3 animate-in zoom-in-95">
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {clarificationSaved ? 'Saved!' : 'Add Note to Summary'}
              </button>
            </div>
          </form>
        ) : (
          ...
          >
            ...
            <span>Need to change or clarify something? Tap to add note</span>
          </button>
        )}
        ...
        <div className="pt-2">
          <button
            ...
            className="w-full py-4 px-8 bg-teal-600 hover:bg-teal-700 active:bg-teal-800 disabled:bg-teal-400 text-white font-bold text-base rounded-2xl shadow-lg shadow-teal-700/20 transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
            {isSubmitting ? (
              <>
                <RefreshCw className="w-5 h-5 animate-spin text-white" />
                <span>Transmitting Encrypted Record to OPD Room 104...</span>
              </>
            ) : (
              <>
                <span>Looks Correct — Send to Doctor's Queue</span>
